 ...
 * Driven entirely by system properties set by the Gradle `vrtTest`/`vrtTestDocker` tasks.
 */
class VrtHarness {
    ...
    fun start() {
        try {
            ...
            // Avoid leaking a started container / Playwright process on partial failure.
            runCatching { close() }
            throw t
        }
    }
    ...
    fun check(scenario: Scenario): String? {
        ...
        try {
            page.setContent(buildPage(scenario))
    ...
    private fun buildPage(scenario: Scenario): String {
        val fragment = createHTML().div {
            id = "capture"
            classes = setOf("inline-block", "p-4")
